...
from utils import generate_api_link # Import generate_api_link
...
def save_pagination_data(unique_name: str, pagination_data):
    ...
    return api_link # Return the api_link
...
def paginate_urls(unique_names: List[str], selected_model: str, indication: str, urls:List[str]):
    ...
    for uniq,current_url in zip(unique_names, urls):
        ...
        # store
        api_link = save_pagination_data(uniq, pag_data)
        pagination_results.append({"unique_name": uniq,"pagination_data": pag_data, "api_link": api_link})
