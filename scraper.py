...
from utils import  generate_unique_name, generate_api_link # Import generate_api_link
...
def save_formatted_data(unique_name: str, formatted_data):
    api_link = generate_api_link()  # Generate the API link
...
def scrape_urls(unique_names: List[str], fields: List[str], selected_model: str):
    ...
    for uniq in unique_names:
        ...
        # store
        api_link = save_formatted_data(uniq, parsed) # Get the API link
        parsed_results.append({"unique_name": uniq,"parsed_data": parsed, "api_link": api_link})
